...
class VoiceAudioSc extends StatefulWidget {
  const VoiceAudioSc({Key key}) : super(key: key);

  @override
  State<VoiceAudioSc> createState() => _VoiceAudioScState();
}

class _VoiceAudioScState extends State<VoiceAudioSc> {
  ...
  Widget tile({@required int index, @required bool isplaying}) {
    return ListTile(
      ...
      title: Text(
        textLimit(
            text: audiofiles[index].sendername, max: 50, morecharacter: ''),
      ),
  ...
  audiotileSub(int index) {
    return Padding(
      padding: EdgeInsets.only(top: size(context).width / 90),
      child: Text(
        '0:34:45',
        style: TextStyle(
